...
#include <QDebug>
#include <string>
CheckersGame::CheckersGame(QTcpSocket *out, CheckersPicture *outpic)
{
    this->readWriteSocket = out;
    this->pic = outpic;
    QObject::connect(this->readWriteSocket,SIGNAL(readyRead()),this,SLOT(parser()));
    connect(this,SIGNAL(pingju()),this,SLOT(pingle()));
...
void CheckersGame::startNewGame(uint8 color) {
    ...
    gamerunning = true;
    if(computercolor == WHITE) {qDebug()<<65;
        pic->thinking = true;qDebug()<<65.5;
        pic->update();qDebug()<<66;
        emit stateChanged(current);qDebug()<<67;
        pp(current,computercolor);qDebug()<<68;
    }
    else {
...
void CheckersGame::parser()
{
    ...
    if(info == "peaceback0")
    {
        QMessageBox::information(NULL, "SUDOtree", "refused", QMessageBox::Yes , QMessageBox::Yes);

        return;
    }
    if(info == "peace")
    {
        QMessageBox::StandardButton rb = QMessageBox::question(NULL, "Sue for peace", "Do you want to accept it?", QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
        if(rb == QMessageBox::Yes)
        {
            QString ss = "peaceback1";
            readWriteSocket->write(ss.toStdString().c_str(),strlen(ss.toStdString().c_str()));
            gamerunning = false;
            emit gameEnded(0);
        }
        else
        {
            hehao = 40;
            QString ss = "peaceback0";
            readWriteSocket->write(ss.toStdString().c_str(),strlen(ss.toStdString().c_str()));
            QMessageBox::information(NULL, "SUDOtree", "Game going on", QMessageBox::Yes , QMessageBox::Yes);
        }
        return;
    }
    if(info == "pingju")
    ...
    {
        auto node = list.item(i);
        auto element = node.toElement();
        auto child_list = element.childNodes();
        int size = child_list.count();
        for(int j=0;j<size;++j)
        {
            QDomNode child_dom_node = child_list.item(j);
            QDomElement child_element = child_dom_node.toElement();
            out[jishu] = child_element.text().toInt();
            ++jishu;
        }
    }
    jishu = 0;
...
{
    bool move = false;
    pp(current,computercolor);
    move = this->move( point(out[0],out[1],MOVEDFROM) , point(out[2],out[3],MOVEDTO) );
    if(!move)
        qDebug()<<"oppo wrong";
    --hehao;
    if(hehao == 0)
...
void CheckersGame::secondClick(int i, int j)
{
    ...
    if( !move ) {
        ...
        if(gamerunning)
        {
            ...
            sx = i;
            sy = j;
            QString ss = "<?xml version=\"1.0\" encoding=\"utf-8\"?>"
                    "<clicks name=\"Qt\">"
                     "<click id=\"1\">"
                      "<i>"+ QString::number(fx)+"</i>"
                      "<j>"+QString::number(fy)+"</j>"
                     "</click>"
                     "<click id=\"2\">"
                      "<i>"+QString::number(sx)+"</i>"
                      "<j>"+QString::number(sy)+"</j>"
                     "</click>";
            readWriteSocket->write(ss.toStdString().c_str(),strlen(ss.toStdString().c_str()));
            //readWriteSocket->waitForBytesWritten();
            qDebug()<<fx<<' '<<fy<<' '<<sx<<' '<<sy<<"dasd";
            --hehao;
            if(hehao == 0)
            {
                emit pingju();
...
void CheckersGame::peace()
{
    if(pic->thinking == false)
    {
        return;
    }
    QString ss = "peace";
...
void CheckersGame::lose()
{
    ...
    readWriteSocket->write(ss.toStdString().c_str(),strlen(ss.toStdString().c_str()));
    gamerunning = false;
    emit gameEnded( computercolor );
}


void CheckersGame::pingle()
{
    QString ss = "pingju";
    readWriteSocket->write(ss.toStdString().c_str(),strlen(ss.toStdString().c_str()));
    gamerunning = false;
